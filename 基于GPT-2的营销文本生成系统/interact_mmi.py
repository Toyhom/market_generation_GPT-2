...
def set_interact_args():
    ...
    parser.add_argument('--voca_path', default='vocabulary/vocab_small_原版.txt', type=str, required=False, help='选择词库')
    ...
    parser.add_argument('--batch_size', type=int, default=2, help='批量生成response，然后经过MMI模型进行筛选')
...
#    mmi_model.eval()
    ...
    try:
        text = '女童演出服装花童公主裙婚纱裙子钢琴礼服裙婚礼伴娘幼儿园表演唱歌比赛白色婚纱礼服，唯美艺术之作，粉色，白色，天使小精灵'
        #text = text.replace(' ','')
        ...
        input_ids += [tokenizer.convert_tokens_to_ids(word) for word in list(jieba.cut(text))]
...
#            input_ids.append(tokenizer.sep_token_id)
        ...
        for _ in range(args.max_len):
            ...
            for index in range(args.batch_size):
                for token_id in [token_ids[index] for token_ids in generated]:
...
#            print(next_token_logits)
            ...
            jingyongci = [str(w) for w in range(10)] + [chr(w) for w in range(ord('a'), ord('z') + 1)] + [chr(w) for w in range(ord('A'), ord('Z') + 1)]
            
#            print(jingyongci)
            
            # 对于[UNK]的概率设为无穷小，也就是说模型的预测结果不可能是[UNK]这个token
            for ti in range(len(next_token_logits)):
                next_token_logits[ti][tokenizer.convert_tokens_to_ids('[UNK]')] = -float('Inf')
                next_token_logits[ti][tokenizer.convert_tokens_to_ids('')] = -float('Inf')
                for w in jingyongci:
                    next_token_logits[ti][tokenizer.convert_tokens_to_ids(w)] = -float('Inf')
#                next_token_logits[ti][tokenizer.convert_tokens_to_ids('，')] = next_token_logits[ti][tokenizer.convert_tokens_to_ids('，')] *2
...
#            
#            
#            mmi_input_id += [tokenizer.convert_tokens_to_ids(word) for word in list(jieba.cut(text))]
#            
#            mmi_input_id.append(tokenizer.sep_token_id)
#            
...
#                min_loss = loss
#            print(' '.join(tokenizer.convert_ids_to_tokens(response)))
